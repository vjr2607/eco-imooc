...
function startsWith($haystack, $needle)  //vjr - copied from ../block_oaipmh.php
{
...
function parseCourseInstanceConfig($config, $systemConfig){
    ...
    if(!isset($config->c_effort)) {
	$retVal['effort']=0;
    }else{
	$retVal['effort']=$config->c_effort;
    }

    if(!isset($config->c_courseurl)) {
	$retVal['course_url']="";
    }else{
	$retVal['course_url']=$config->c_courseurl;
    }

    if(isset($config->c_enddate)) {
	$retVal['enddate']=date('c',$config->c_enddate);
    }
...
function getAllDataFromMoodle(){
	...
	$data['startdate'] = date('c',$c->startdate); //vjr
	$fopt = $DB->get_record('course_format_options', array('courseid'=>$c->id,'name'=>'numsections')); //vjr
	$data['n_units'] = $fopt->value - 3; //vjr
	...
	$cat = $DB->get_record('course_categories', array('id'=>$c->category)); //vjr
	$data['taxonomy_id'] = $cat->idnumber; //vjr
	$data['taxonomy'] = $cat->name;  //vjr
        if($force_license){
            // Force server license
	...
	$contxt = context_course::instance($c->id);
	$teachers = get_enrolled_users($contxt, 'moodle/course:update', 0, 'u.id,u.firstname,u.lastname,u.email,u.institution','firstname,lastname');
	$data['teachers'] = serialize($teachers);
//"BEGIN:VCARD\nFN:Vitor Rocio\nUID:urn:uuid:eco-imooc-2134\nEMAIL;TYPE=INTERNET:vjr@uab.pt\nORG:Universidade Aberta\nN:Rocio;Vitor\nVERSION:3.0\nEND:VCARD";  //vjr
	$data['effort'] = $moduleCfg['effort']; //vjr
	$data['enddate'] = $moduleCfg['enddate']; //vjr
	$data['course_url'] = $moduleCfg['course_url']; //vjr

...
function reverse_url($url) {
        $spl_url = preg_split("/\./", $url);
        $rev_url = "";
        for ($i=count($spl_url)-1;$i>0;$i--)
           $rev_url = $rev_url.$spl_url[$i].".";
        $rev_url = $rev_url.$spl_url[0];
        return $rev_url;
}
...
function listRecords(/*$dataToDisplay, */ $identifieronly=false, $format="oai_lom"){
    ...
    echo "<OAI-PMH xmlns='http://www.openarchives.org/OAI/2.0/' xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xsi:schemaLocation='http://www.openarchives.org/OAI/2.0/ http://www.openarchives.org/OAI/2.0/OAI-PMH.xsd' xmlns:eco='http://www.ecolearning.eu/xsd/LOM'>\n";
    ...
    $server = $data['server'];
    $rev_server = reverse_url($server); //vjr
    ...
    foreach($data['data'] as $r){
        ...
        if($identifieronly){ // Only identifiers
            ...
            echo ">
            <identifier>oai:{$rev_server}:{$r->uniq_id}</identifier>
            <datestamp>".gmdate("Y-m-d\TH:i:s\Z", $r->unixts)."</datestamp>
            <setSpec>moodle</setSpec>
            </header>";
        }else if($r->deleted == "1"){ // Deleted item

	    if (strpos($r->uniq_id,"_eteach"))
		$eco_type="";
	    else
		$eco_type="<setSpec>EcoBase</setSpec>";
            ...
            //if($r->taxonomy!=""){
                ...
                $taxonomy .= "<taxonPath>
                                <source>
                                    <string language='en'>ECO Area of Interests</string>  //vjr
                                </source>";
               // foreach($el as $e){
                    $taxonomy .= "<taxon>
                                    <id>{$r->taxonomy_id}</id>
                                    <entry>
                                    <string language='en'>{$r->taxonomy}</string>
                                    </entry>
                                    </taxon>";
                //}
                $taxonomy .= "</taxonPath>";
		...
		$teachers .= '<contribute>
                        ...
                        <entity>'."<![CDATA[BEGIN:VCARD\r\nFN:".$u->firstname.' '.$u->lastname."\r\nUID:urn:uuid:eco-imooc-".$u->id."\r\nEMAIL;TYPE=INTERNET:".$u->email."\r\nORG:".$u->institution."\r\nN:".$u->lastname.';'.$u->firstname."\r\nVERSION:3.0\r\nEND:VCARD]]></entity>
                    </contribute>";
	    }

	    $studyLoad = round($r->effort/((strtotime($r->enddate)-strtotime($r->startdate))/60/60/24/7));
	    if (strpos($r->uniq_id,"_eteach"))
		$eco_type="";
	    else
		$eco_type="<setSpec>EcoBase</setSpec>";
		...
		<eco:nrOfUnits>{$r->n_units}</eco:nrOfUnits>
		<eco:startDate>{$r->startdate}</eco:startDate>
		<eco:endDate>{$r->enddate}</eco:endDate>
		<eco:studyLoad>{$studyLoad}</eco:studyLoad>
		<eco:courseImage>
		    <eco:courseUrl>
			http://eco.imooc.uab.pt/images/courses/{$r->uniq_id}/image.jpg
		    </eco:courseUrl>
		</eco:courseImage>
                </general>
		    ...
		    {$teachers}
                ...
                <technical>
                    <location>{$r->course_url}</location>
                </technical>
                <educational>
		    <typicalLearningTime>
			<duration>PT{$r->effort}H</duration>
		    </typicalLearningTime>
		</educational>
                <rights>

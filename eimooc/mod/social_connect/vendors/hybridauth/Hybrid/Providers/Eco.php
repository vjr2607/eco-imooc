 ...
 */
class Hybrid_Providers_Eco extends Hybrid_Provider_Model_OAuth2
{
	...
	*/
	function initialize() 
	{
		...
		$this->api->authorize_url  = "https://idp.ecolearning.eu/authorize";
		$this->api->token_url      = "https://idp.ecolearning.eu/token";
...
/*		if (strpos($this->scope, 'email') !== false) {
			$verified = $this->api->api( "http://ecoidp.test.reimeritsolutions.nl/userinfo" );
		...
		$response = $this->api->api( "https://idp.ecolearning.eu/userinfo" );
		...
		$this->user->profile->language      = (property_exists($response,'locale'))?$response->language:"";
...
/*		if (property_exists($response, 'emails')) {
		...
		Hybrid_Logger::debug(serialize($this->user->profile));

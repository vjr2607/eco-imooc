...
class OAuth2Client
{
	...
	public function authenticate( $code )
	{
		...
		//array_push($this->curl_header, "Content-Type: application/x-www-form-urlencoded"); //vjr
		//array_push($this->curl_header, "Authorization: Basic " . base64_encode($this->client_id . ":" . $this->client_secret)); //vjr
	...
	*/
	public function api( $url, $method = "GET", $parameters = array() ) 
	{
		...
		array_push($this->curl_header, "Authorization: Bearer " . $this->access_token);
		$response = null;
...
//		Hybrid_Logger::info( "Enter OAuth2Client::request( $url )" );
		Hybrid_Logger::info( "Enter OAuth2Client::request( $url , $type )" );  //vjr
		...
		curl_setopt($ch, CURLINFO_HEADER_OUT, 1);  //vjr
...
//		Hybrid_Logger::debug( "OAuth2Client::request(). dump request info: ", serialize( curl_getinfo($ch) ) );
		Hybrid_Logger::debug( "OAuth2Client::request(). dump request info: ", curl_getinfo($ch, CURLINFO_HEADER_OUT) );  //vjr
